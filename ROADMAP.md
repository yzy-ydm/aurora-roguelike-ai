# ROADMAP.md

> Aurora-Roguelike-AI 项目开发路线图

最后更新：2026-07-13

---

## 项目总览

本项目分为9个主要阶段，从项目初始化到最终论文答辩。

---

## Phase 1：项目初始化 ✅

**状态：** 已完成

**目标：** 建立项目基础结构

**完成内容：**
- [x] 项目目录结构设计
- [x] README.md 项目说明文档
- [x] .gitignore 配置
- [x] 开发规范文档
- [x] 基础工程规范

---

## Phase 2：Git工程管理 ✅

**状态：** 已完成

**目标：** 建立版本管理体系

**完成内容：**
- [x] Git仓库初始化
- [x] GitHub私有仓库创建
- [x] main分支管理
- [x] 提交规范建立

---

## Phase 3：MySQL数据库设计 ✅

**状态：** 已完成

**目标：** 设计完整的游戏数据库

**完成内容：**
- [x] 数据库 aurora_game 创建
- [x] 8张核心数据表设计
- [x] 测试数据导入
- [x] 数据库设计文档

**数据表：**
- users（用户表）
- player_profiles（玩家档案表）
- weapons（武器表）
- monsters（怪物表）
- events（事件表）
- maps（地图表）
- game_saves（存档表）
- ai_generations（AI生成记录表）

---

## Phase 4：后端系统开发

**状态：** 已完成（4.6.3完成）

**目标：** 实现完整的FastAPI后端服务

### Phase 4.1：数据库连接 ✅

**状态：** 已完成

**完成内容：**
- [x] FastAPI应用初始化
- [x] SQLAlchemy ORM配置
- [x] MySQL数据库连接
- [x] 环境变量配置
- [x] 健康检查接口（/health）

### Phase 4.2：用户认证系统 ✅

**状态：** 已完成

**完成内容：**
- [x] 用户注册接口（POST /api/auth/register）
- [x] 用户登录接口（POST /api/auth/login）
- [x] JWT Token认证
- [x] bcrypt密码加密

### Phase 4.3：玩家角色系统 ✅

**状态：** 已完成

**目标：** 实现玩家角色管理

**完成内容：**
- [x] player_profiles 模型（app/models/player_profile.py）
- [x] 创建角色接口（POST /api/player/profile）
- [x] 查询角色信息接口（GET /api/player/profile）
- [x] 更新角色信息接口（PUT /api/player/profile）
- [x] JWT认证集成

### Phase 4.4：武器系统 ✅

**状态：** 已完成

**目标：** 实现武器数据管理

**完成内容：**
- [x] weapons 模型（app/models/weapon.py）
- [x] player_weapons 模型（app/models/player_weapon.py）
- [x] 武器列表查询接口（GET /api/weapons）
- [x] 武器详情查询接口（GET /api/weapons/{id}）
- [x] 玩家武器查询接口（GET /api/player/weapons）
- [x] 添加玩家武器接口（POST /api/player/weapons）

### Phase 4.5：怪物系统 ✅

**状态：** 已完成

**目标：** 实现怪物数据管理

**完成内容：**
- [x] monsters 模型（app/models/monster.py）
- [x] 怪物列表查询接口（GET /api/monsters）
- [x] 怪物详情查询接口（GET /api/monsters/{id}）

### Phase 4.6：游戏数据接口

**状态：** 进行中（4.6.1完成）

**目标：** 实现游戏核心数据接口

**Phase 4.6.1：游戏存档系统 ✅**

**完成内容：**
- [x] game_saves 模型（app/models/game_save.py）
- [x] 创建存档接口（POST /api/game/save）
- [x] 查询所有存档接口（GET /api/game/save）
- [x] 查询指定存档接口（GET /api/game/save/{slot}）
- [x] 更新存档接口（PUT /api/game/save/{slot}）

**Phase 4.6.2：地图数据接口 ✅**

**完成内容：**
- [x] maps 模型（app/models/map.py）
- [x] 地图列表查询接口（GET /api/maps）
- [x] 地图详情查询接口（GET /api/maps/{id}）

**Phase 4.6.3：事件数据接口 ✅**

**完成内容：**
- [x] events 模型（app/models/event.py）
- [x] 事件列表查询接口（GET /api/events）
- [x] 事件详情查询接口（GET /api/events/{id}）

---

## Phase 5：Godot客户端开发

**状态：** 进行中（5.1完成）

**目标：** 实现游戏客户端基础框架

**Phase 5.1：客户端基础框架 ✅**

**完成内容：**
- [x] Godot项目初始化（client/project.godot）
- [x] API配置模块（client/scripts/api/api_config.gd）
- [x] Token管理模块（client/scripts/api/token_manager.gd）
- [x] HTTP客户端模块（client/scripts/api/api_client.gd）
- [x] 登录/注册界面（client/scenes/login/）
- [x] 玩家信息界面（client/scenes/main/）

**Phase 5.2：基础游戏场景 ✅**

**完成内容：**
- [x] 游戏主场景框架（client/scenes/game/）
- [x] 玩家节点显示（CharacterBody2D）
- [x] HUD系统（昵称、等级、生命值、金币）
- [x] 场景管理器（scene_manager.gd）
- [x] 资源管理器框架（resource_manager.gd）

**Phase 5.3：玩家基础控制 ✅**

**完成内容：**
- [x] 键盘输入处理（WASD + 方向键）
- [x] 玩家移动逻辑（CharacterBody2D + move_and_slide）
- [x] 摄像机跟随（Camera2D跟随玩家）
- [x] 基础碰撞检测（测试边界碰撞墙）

**Phase 5.4：游戏资源加载系统 ✅**

**完成内容：**
- [x] 武器数据模型（weapon_data.gd）
- [x] 怪物数据模型（monster_data.gd）
- [x] 地图数据模型（map_data.gd）
- [x] 事件数据模型（event_data.gd）
- [x] 资源加载服务（resource_service.gd）
- [x] HUD资源统计显示

**Phase 5.5：基础游戏资源展示系统 ✅**

**完成内容：**
- [x] 资源中心场景（resource_center.tscn）
- [x] 武器列表和详情展示
- [x] 怪物列表和详情展示
- [x] 地图列表和详情展示
- [x] 事件列表和详情展示
- [x] 游戏场景资源中心入口

**Phase 5.6：游戏循环系统 ✅**

**完成内容：**
- [x] 游戏状态管理器（game_state_manager.gd）
- [x] 存档服务（save_service.gd）
- [x] 游戏流程控制器（game_flow_controller.gd）
- [x] 登录→游戏流程集成
- [x] 退出保存功能

**Phase 5.7：基础游戏交互系统 ✅**

**完成内容：**
- [x] 暂停菜单系统（ESC打开/关闭）
- [x] 设置系统（音量配置）
- [x] 存档选择界面（3个槽位）
- [x] ConfigFile本地配置保存
- [x] Godot暂停机制集成

**Phase 5.8：基础游戏世界构建 ✅**

**完成内容：**
- [x] 世界管理器（world_manager.gd）
- [x] 房间管理器（room_manager.gd）
- [x] 地图渲染器（map_renderer.gd）
- [x] 房间数据模型（room_data.gd）
- [x] 世界和房间场景
- [x] 游戏场景集成

**Phase 5.9：游戏实体基础框架 ✅**

**完成内容：**
- [x] Entity基础类（entity.gd）
- [x] EntityManager（entity_manager.gd）
- [x] PlayerEntity（player_entity.gd）
- [x] 实体唯一ID系统
- [x] 实体位置管理

**Phase 5.10：基础游戏对象交互框架 ✅**

**完成内容：**
- [x] InteractiveObject基础类
- [x] InteractionManager交互管理器
- [x] InteractionDetector交互检测器
- [x] InteractionHint交互提示UI
- [x] E键交互输入配置
- [x] 游戏场景集成交互系统

**Phase 5.11：基础游戏对象实例化系统 ✅**

**完成内容：**
- [x] GameObject基础类（game_object.gd）
- [x] ObjectManager（object_manager.gd）
- [x] TestChest测试对象（test_chest.gd）
- [x] 游戏场景集成对象系统
- [x] 测试宝箱实例化和交互

**Phase 6.1：玩家资源系统 ✅**

**完成内容：**
- [x] WeaponObject（武器拾取对象）
- [x] InventoryManager（背包管理）
- [x] EquipmentManager（装备管理）
- [x] 武器拾取交互流程
- [x] 与player_weapons API同步

**Phase 6.2：基础战斗系统准备 ⬜**

**计划内容：**
- [ ] 战斗状态管理
- [ ] 伤害计算框架
- [ ] 战斗回合系统
- [ ] 怪物实体接入

---

## Phase 6：核心玩法开发

**状态：** 待开发

**目标：** 实现Roguelike核心玩法

**计划内容：**
- [ ] 随机地图生成
- [ ] 玩家移动系统
- [ ] 战斗系统
- [ ] 道具/武器系统
- [ ] 怪物AI
- [ ] 游戏流程控制
- [ ] 存档/读档功能

---

## Phase 7：AI动态内容生成

**状态：** 待开发

**目标：** 集成云端AI实现动态内容生成

**计划内容：**
- [ ] AI服务抽象层
- [ ] DeepSeek API接入
- [ ] AI生成武器属性
- [ ] AI生成怪物描述
- [ ] AI生成随机事件
- [ ] AI生成剧情文本
- [ ] 生成结果缓存

---

## Phase 8：测试与优化

**状态：** 待开发

**目标：** 系统测试和性能优化

**计划内容：**
- [ ] 单元测试
- [ ] 接口测试
- [ ] 集成测试
- [ ] 性能优化
- [ ] Bug修复
- [ ] 用户体验优化

---

## Phase 9：论文与答辩

**状态：** 待开发

**目标：** 完成毕业论文和答辩准备

**计划内容：**
- [ ] 论文撰写
- [ ] 系统截图
- [ ] 演示视频
- [ ] 答辩PPT
- [ ] 答辩准备

---

## 里程碑

| 里程碑 | 目标日期 | 状态 |
|--------|----------|------|
| M1: 项目初始化完成 | - | ✅ 已完成 |
| M2: 数据库设计完成 | - | ✅ 已完成 |
| M3: 后端基础完成 | - | ✅ 已完成 |
| M4: 后端API完成 | - | ⬜ 进行中 |
| M5: 客户端基础完成 | - | ⬜ 待开发 |
| M6: 核心玩法完成 | - | ⬜ 待开发 |
| M7: AI集成完成 | - | ⬜ 待开发 |
| M8: 项目测试完成 | - | ⬜ 待开发 |
| M9: 论文答辩完成 | - | ⬜ 待开发 |

---

## 技术债务

暂无。

---

## 风险项

| 风险 | 影响 | 应对措施 |
|------|------|----------|
| AI API不稳定 | AI功能受限 | 设计降级方案，本地缓存 |
| Godot学习曲线 | 客户端开发延迟 | 参考示例项目，简化实现 |
| 时间紧张 | 功能缩减 | 优先核心功能，AI功能可选 |
